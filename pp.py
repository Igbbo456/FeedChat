...
def get_notifications(username):
    c.execute("SELECT id, message, timestamp FROM notifications WHERE username=? AND seen=0 ORDER BY id DESC", (username,))
    return c.fetchall()
...
with st.sidebar:
    ...
    if st.session_state["user"]:
        ...
        notifs = get_notifications(st.session_state["user"])
        if notifs:
            st.warning(f"🔔 {len(notifs)} new notifications")
            if st.button("Mark all read"):
                mark_notifications_seen(st.session_state["user"])
                st.rerun()
        ...
        if st.session_state["page"] == "Feed":
            ...
            for pid, uname, msg, ts in posts:
                ...
                if c1.button(f"👍 Like {pid}"):
                    ...
                    add_notification(uname, f"{st.session_state['user']} liked your post")
                    st.rerun()
                comment = c2.text_input(f"Comment on {pid}", key=f"c{pid}")
                if c2.button(f"Add {pid}"):
                    if comment.strip():
                        add_comment(pid, st.session_state["user"], comment)
                        add_notification(uname, f"{st.session_state['user']} commented on your post")
                        st.rerun()
            ...
            if st.button("Send"):
                if msg.strip():
                    ...
                    add_notification(receiver, f"New message from {st.session_state['user']}")
                    st.rerun()
